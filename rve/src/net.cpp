...
static struct {
    ...
    uint32_t  hdr_pos;   // how many length bytes have been read
    uint8_t  *data;      // payload buffer (malloc'd when length is known)
    uint32_t  data_pos;  // how many payload bytes have been read
    bool      hdr_done;  // true once all 4 length bytes are received
    int       reads;     // diagnostic read-count for the current packet
} s_recv;
...
void net_init(const char *path, bool server)
{
    ...
    // Non-blocking I/O so net_recv never stalls the emulation loop.
...
void net_send(uint8_t *data, uint32_t len)
{
    ...
    for (uint32_t i = 0; i < 4; i++) {
        ...
        if (write(net_fd_conn, &b, 1) < 0) {
            ...
            // On error, mark the connection dead so future calls are no-ops.
            close(net_fd_conn);
            net_fd_conn = -1;
            return;
        }
...
bool net_recv(uint8_t **data_out, uint32_t *len_out)
{
    ...
    while (!s_recv.hdr_done && s_recv.hdr_pos < 4) {
        ...
        if (n == 1) {
            ...
            if (s_recv.hdr_pos == 0) return false; // nothing pending, normal idle
            // Partial header → connection error.
    ...
    if (!s_recv.hdr_done) {
        ...
        // Length header complete.
        if (s_recv.len.i32 <= 0) {
            // Zero-length or corrupt → drop.
    ...
    *len_out  = (uint32_t)s_recv.len.i32;
    ...
    // Reset without freeing *data_out — caller owns it now.
    s_recv.data     = nullptr;
    s_recv.hdr_pos  = 0;
    s_recv.hdr_done = false;
    s_recv.data_pos = 0;
